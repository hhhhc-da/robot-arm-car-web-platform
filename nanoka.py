...
types = ['cross_1', 'cross_2', 'cross_3', 'cross_4', 'cross_5', 'cross_6',
         'path_1', 'path_2', 'path_3', 'path_4', 'path_5', 'path_6', 'path_7']
figure_size = (224, 224)
arg_kwargs = {
    'host': "localhost",
    'port': 3308,
...
# Cache
box_cache, yolo_cache, obj_cache = None, np.zeros(
    [640, 640, 3]), np.zeros([640, 640, 3])
...
def fcn_write_box(im):
    ...
    for i in range(mask.shape[0]):
        mask_figure = cv2.threshold(
            mask[i, :, :], 8, 255, cv2.THRESH_BINARY)[1]

        contours, _ = cv2.findContours(mask_figure.astype(
    ...
    for i in range(len(boxes)):
        ...
        if i < 6:
            box_img = cv2.rectangle(
                box_img, (x, y), (x + w, y + h), (0, 0, 255), 2)
            box_img = cv2.putText(
                box_img, types[i], (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
        else:
            box_img = cv2.rectangle(
                box_img, (x, y), (x + w, y + h), (255, 0, 0), 2)
            box_img = cv2.putText(
                box_img, types[i], (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1, cv2.LINE_AA)
...
@app.route('/arm_exec', methods=['POST'])
def arm_exec():
    raw_data = request.data.decode()
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.connect(('192.168.43.101', 8080))
    ...
    try:
        ...
        ret = 1
        info = 'Fatal error occured (arm_exec)'
    finally:
        s.close()

    if ret != 0:
        return jsonify({'code': ret, 'info': info}), 500
    else:
        return jsonify({'code': ret, 'info': info}), 200
...
if __name__ == "__main__":
    ...
    model = DetectMultiBackend(
        'nanoka_v1.pt', data='dataset/nanoka_v1/data.yaml')
    # Load YOLOv5 model (Object detect)
    obj_model = DetectMultiBackend(
        'nanoka_v2.pt', data='dataset/nanoka_v2/data.yaml')
